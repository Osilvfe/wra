...
if [ ! -d "rootfs" ]; then
    _notice_ "Copy Debian trixie...to "$(pwd)"/rootfs for run"
...
apt install golang-go liblz4-dev libusb-1.0-0-dev libprotobuf-dev libunwind-dev protobuf-compiler libgtest-dev libbrotli-dev
cmake ..
cmake --build . -j
